...
class ReplayBuffer:
    ...
    def sample_batch(self, batch_size=32):
        ...
        return {k: torch.as_tensor(v, device=device, dtype=torch.float32) for k,v in batch.items()}
...
def td3(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    def get_obs(env, a=None):
        ...
        y = np.concatenate( [time_step.observation[key] for key in list(time_step.observation.keys())] )      
        return y
